...
type searchType = "Tất cả" | "Lọc" | "Phim" | "Truyền hình" | "Mọi người";

const SearchStatic = () => {
  const pathname = usePathname();
  const searchTerm = useContext(SearchContext);
  const [showOptions, setShowOptions] = useState(false);
  const [searchType, setSearchType] = useState<searchType>("Lọc");
    ...
    <>
      {pathname === "/search" ? (
        <section>
          <div className="border-[0.4px] border-opacity-20 w-64 transition-all duration-150 ease-in focus-within:w-80 focus-within:border-opacity-80 pl-3   border-_light_white  flex items-center  font-light gap-3 h-8 text-_light_white rounded-2xl ">
            ...
            >
              ...
              >
                <ul>
                  <List text="Tất cả" onClick={handleClick} />
                  <List text="Phim" onClick={handleClick} />
                  <List text="Truyền hình" onClick={handleClick} />
                  <List text="Mọi người" onClick={handleClick} />
                </ul>
              </div>
            </section>
          </div>
        ...
        >
          ...
          <p className="text-xs">Tìm kiếm mọi thứ ở đây...</p>
        </Link>
      )}
    </>
  );
