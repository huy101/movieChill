...
const AllUsers = () => {
    ...
    const fetchNextPage = async (page: number) => {
      ...
      try {
        ...
        if (error.response.status === 404) {
          setHasMore(false);
          toast.error("Không có người dùng nào");
        }
      } finally {
        setLoading(false);
      }
  ...
  return (
    ...
    >
      ...
      <>
        ...
        {searchTerm.trim() !== "" &&
          ...
          FilteredUsers?.length === 0 && (
            <p className="text-white text-center">Không tìm thấy người dùng</p>
          )}
        {searchTerm.trim() !== ""
      ...
      {!loading && !hasMore && <p className="text-white text-center">Hết</p>}
    </div>
  );
};
