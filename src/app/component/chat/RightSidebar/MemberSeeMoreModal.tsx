...
const MemberSeeMoreModal: React.FC<UserExtended> = ({
  ...
  const createAccessChat = useMutation(
    (id: string) => userApis.createAccessChat(id),
    {
      ...
      onError: (data) => {
        toast.error("Không tạo được quyền truy cập trò chuyện!");
      },
    }
  );
      ...
      <ul>
        {_id !== user.id && (
          ...
          >
            ...
            <span className="font-light ">Tin nhắn</span>
          </li>
        )}
        <Link
        ...
        >
          ...
          <span className="font-light ">Trang cá nhân</span>
        </Link>
        ...
        {user.id === owner && _id !== user.id && (
          ...
          >
            ...
            <span className="font-light text-red-400 ">Xóa thành viên</span>
          </li>
        )}
        ...
        {user.id === owner && _id !== user.id && (
          <li className="flex gap-5 hover:bg-neutral-800 text-base transition-colors duration-200 ease-linear cursor-pointer py-2 items-center px-4 text-neutral-300">
            ...
            <span className="font-light text-red-400 ">Chặn</span>
          </li>
        )}
        {user.id === owner && _id !== user.id && (
          <li className="flex gap-5 hover:bg-neutral-800 text-base transition-colors duration-200 ease-linear cursor-pointer py-2 items-center px-4 text-neutral-300">
            <MdGppGood className="text-green-400 text-xl" />
            <span className="font-light text-green-400 ">Ủy quyền quản trị</span>
          </li>
        )}
        {user.id === _id && (
          <li className="flex gap-5 hover:bg-neutral-800 text-base transition-colors duration-200 ease-linear cursor-pointer py-2 items-center px-4 text-neutral-300">
            <MdBlockFlipped className="text-red-400 text-xl" />
            <span className="font-light text-red-400 ">Rời nhóm</span>
          </li>
        )}
      </ul>
    </div>
