...
const SingleMessage = () => {
  ...
  if (!user.isUserAuthenticated) {
    return (
      <div className="flex items-center justify-center h-screen">
        {" "}
        <h1 className="text-2xl text-neutral-400">Vui lòng đăng nhập để tiếp tục</h1>
      </div>
    );
  }
  ...
  if (!ChatId) {
    return (
      <div className="flex items-center justify-center h-screen">
        {" "}
        <h1 className="text-2xl text-neutral-400">Không có cuộc trò chuyện nào được chọn</h1>
      </div>
    );
  }
  ...
  const handleMsgSend = () => {
    if (!message || message.trim() === "") {
      return toast.error("Tin nhắn không được để trống");
    }
